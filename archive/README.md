...
|---|---|---|---|
| `wallsys_old/` | C# | Yes — `.exe` files | Predecessor to `wallsys/`. Two separate binaries for nature/tech wallpaper pools. Simple random picker (no shuffle queue). Still works — compile with `build.bat`, populate `assets/nature/` and `assets/tech/`, then map hotkeys (e.g. <kbd>Alt+Shift+N</kbd> for nature, <kbd>Alt+Shift+T</kbd> for tech) to each `.exe`. |
